...
// ================================================================
/*
          _     _           MPU6050 MultiReader UDP Sender
         /\`\ /`/\          Created by Bingcheng HU on 18/3/12.
         \/\ V /\/          this program should be uploaded to ESP8266 DevKit  (NodeMCU)
           /6 6\            For more you can see https://github.com/nodemcu/nodemcu-devkit-v1.0
          (= Y =)           WiFi SSID looks like: “MPU6050-AABDC&@”, and password is “12345678”
          /`'^'`\           If the initialization is successful, the LED will blink three times;
         / .-*-. \          If it fails, the LED on the board will keep flashing.
        (_/*=*=*\_)         D4-SCL and D3-SDA are used for I2C, GPIO16 for LED, RX-D9, TX-D10,
         /|*=*=*|\          So 11 ports in total, 6 ports available yet.
      ___\ ’-*-’ /___       These ports are 4 ,5, 10, 12, 13, 14, 15.
     (((____/^\____)))      If this program cannot run:
                            Setup arduino IDE for esp8266 from 
                            http://www.yfrobot.com/thread-11797-1-1.html
                            Setup Muxing Around With The CD74HC4067 
                            http://bildr.org/2011/02/cd74hc4067-arduino/
                            then select board "NodeMCU 1.0"
                            Copyright © 2018 Bingcheng1998@qq.com.
*/
