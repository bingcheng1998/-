...
//---------------测试用-------------------
static int M0 [6] = {-1213,771,979,125,29,-10};
...
float server_goto(float angle){
  float real_angle = ((angle-90)/90)*(66) + 38;
  currentAngle = angle;
  myservo.write(real_angle);
  return real_angle;
}
...
void loop() {
//  delay(500);
  if(millis() - start_millis < wait_millis){
    currentAngle = 180;
  } else if(millis() - last_millis > diff_millis){
    last_millis = millis();
    if(currentAngle < 90 && dir_flag == true){
     dir_flag = false;
     stop_flag = true;
     stop_millis = millis();
//     delay(10000);
    } else if(currentAngle > 180 && dir_flag == false){
      dir_flag = true;
      stop_flag = true;
      stop_millis = millis();
//      delay(10000);
    }
    if(stop_flag == true && millis() - stop_millis > stop_time){
      stop_flag = false;
...
//      delay(diff_millis);
  ...
  for (int i = 0; i < mpuNum; ++i)
  {
    ...
    if ((mpuIntStatus[i] & 0x10) || fifoCount[i] == 1024) {
      ...
      //   Udp.write(teapotPacket[j]);
      // }
      teapotPacket[11] = currentAngle-90;
